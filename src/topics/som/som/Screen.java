...
/*  
  ...
  are called by the SOM_Thread class in SomsApplet.java. The interesting
  procedures here are: 
    public void init_v_map() 
    public IntegerPoint get_bmu(FloatPoint input) 
    public void scale_neighbors(IntegerPoint loc, FloatPoint factual, float t2) 
...
public class Screen extends JPanel {
  ...
  FloatPoint v_weights[][]   = new FloatPoint [50][50];

  //Randomly selected colors
  FloatPoint v_samples[] = new FloatPoint [50];
...
*/
  public float get_dist(FloatPoint imap, FloatPoint iactual) {
    FloatPoint d= new FloatPoint();
    d = imap.sub(iactual);
...
*/
  public float fget_dist(FloatPoint imap, FloatPoint iactual) {
    FloatPoint d= new FloatPoint();
    d = imap.sub(iactual);
    d.set(d.x*d.x,d.y*d.y,d.z*d.z);
  ...
  public void init_Screen() {
    ...
    for (int loop=0; loop<HEIGHT; loop++)
      for (int loop2=0; loop2<WIDTH; loop2++) {
        v_weights[loop2][loop] = new FloatPoint();
        vrscr[loop2][loop] = new Color(0);
        }

    for (int loop=0; loop<MAX_ACTUAL; loop++)
      v_samples[loop] = new FloatPoint();
...
*/
  public void init_v_weights() {
    ...
    if (INIT_STYLE==2) {
      FloatPoint center = new FloatPoint((float)(WIDTH),(float)(HEIGHT),0.0f);
      FloatPoint outer = new FloatPoint(0.0f,0.0f,0.0f);
      ...
      FloatPoint rcenter = new FloatPoint((float)((Math.cos(theta1))*H4),(float)((Math.sin(theta1))*H4),0.0f);
      FloatPoint gcenter = new FloatPoint((float)((Math.cos(theta2))*H4),(float)((Math.sin(theta2))*H4),0.0f);
      FloatPoint bcenter = new FloatPoint((float)((Math.cos(theta3))*H4),(float)((Math.sin(theta3))*H4),0.0f);
...
*/
  IntegerPoint get_bmu(FloatPoint input) {  
    int match_amt=0;
    int WH=WIDTH*HEIGHT;
    float max_dist=10000.0f;
    IntegerPoint match_list[] = new IntegerPoint[WIDTH*HEIGHT];


    for (int loop=0; loop<WIDTH*HEIGHT; loop++)
      match_list[loop] = new IntegerPoint();
...
*/
  public void scale_neighbors(IntegerPoint loc, FloatPoint factual, float t2) {
    ...
    FloatPoint outer  = new FloatPoint((float)(R2),(float)(R2),0.0f);
    FloatPoint center = new FloatPoint(0.0f,0.0f,0.0f);
    ...
    for (int loop=-R2; loop<R2; loop++)
      for (int loop2=-R2; loop2<R2; loop2++)
        if ((loop+loc.y)>=0 && (loop+loc.y)<HEIGHT && (loop2+loc.x)>=0 && (loop2+loc.x)<WIDTH) {
          ...
          FloatPoint temp = (factual.mult(t)).add(v_weights[loc.x+loop2][loc.y+loop].mult(1.0f-t));
...
*/  
  public void update_rgb() {
    IntegerPoint t = new IntegerPoint();
